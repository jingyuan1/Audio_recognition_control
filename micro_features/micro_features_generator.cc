...
TfLiteStatus InitializeMicroFeatures(tflite::ErrorReporter* error_reporter) {
  ...
  config.filterbank.lower_band_limit = 125.0;//55
  config.filterbank.upper_band_limit = 7500.0;//8000
  config.noise_reduction.smoothing_bits = 10;
...
TfLiteStatus GenerateMicroFeatures(tflite::ErrorReporter* error_reporter,
  ...
  int32_t start = g_latest_audio_timestamp;
  FrontendOutput frontend_output = FrontendProcessSamples(
      &g_micro_features_state, frontend_input, input_size, num_samples_read);
  int32_t end = g_latest_audio_timestamp - start;
  int avg = 0;
  for (int i = 0; i <frontend_output.size; ++i){
     avg += frontend_output.values[i];
  }
  avg = avg/(frontend_output.size+frontend_output.size);
  for (int i = 0; i < frontend_output.size; ++i) {
    ...
    int32_t value =
        (((frontend_output.values[i]-avg) * value_scale) + (value_div / 2)) /
        value_div;
    if (value < 0) {
      value = 0;
    }
